<script lang="ts">
  ...
  let setupData = $state<{ otpauth_url: string, qr_png_b64: string } | null>(null);
  ...
  async function verifySetup() {
    ...
    error = "";
    try {
      const valid = await invoke<boolean>("totp_verify_setup", { code: verificationCode });
      if (valid) {
        isEnabled = true;
        step = 3;
      } else {
  ...
  async function disableTotp() {
    if (!confirm($_("settings.security.totp_disable_confirm"))) return;
    processing = true;
    try {
...
<div class="space-y-6">
  <header class="flex items-center gap-3">
    ...
    <div>
      <h1 class="text-xl font-bold text-text-primary">{$_("settings.security.title")}</h1>
    ...
    <div class="bg-surface/3 border border-surface/8 rounded-2xl p-5 space-y-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-full flex items-center justify-center {isEnabled ? 'bg-success/10 text-success' : 'bg-surface/10 text-text-muted'}">
            <IconShieldCheck size={20} />
          </div>
          <div>
            <p class="text-sm font-semibold text-text-primary">{$_("settings.security.totp_label")}</p>
            <p class="text-xs text-text-muted">{isEnabled ? $_("settings.security.totp_enabled") : $_("settings.security.totp_disabled")}</p>
          </div>
        </div>
      ...
      {#if error}
        <div class="p-3 bg-danger/10 border border-danger/20 rounded-lg flex items-center gap-2 text-danger animate-in fade-in slide-in-from-top-1">
        ...
        <div class="pt-4 border-t border-surface/8 space-y-5 animate-in fade-in slide-in-from-top-2">
          ...
          <div class="flex flex-col items-center gap-4">
            <div class="bg-white p-3 rounded-xl shadow-xl">
              <img src="data:image/png;base64,{setupData.qr_png_b64}" alt="TOTP QR Code" class="w-40 h-40" />
            </div>
          ...
          <div class="space-y-2">
            <label for="totp-verify" class="text-xs font-semibold text-text-secondary">
      ...
      {:else if step === 3}
        <div class="pt-4 border-t border-surface/8 text-center space-y-3 animate-in zoom-in-95">
          ...
          <h2 class="text-lg font-bold text-text-primary">{$_("settings.security.totp_success_title")}</h2>
  ...
  <div class="p-4 bg-accent/5 border border-accent/10 rounded-xl">
    <h3 class="text-sm font-bold text-accent-light mb-1">{$_("settings.security.pro_tip_title")}</h3>

<script lang="ts">
  ...
  import { IconEye, IconEyeOff, IconTrash, IconCopy, IconPlus, IconSearch } from '@tabler/icons-svelte';
...
<div class="max-w-[1100px] mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300">
  ...
  <div class="flex items-center justify-between mb-7 gap-4">
    <div>
      <h1 class="text-[26px] font-bold bg-linear-to-br from-white to-accent-light bg-clip-text text-transparent pointer-events-none">{$_("dashboard.title")}</h1>
      <p class="text-text-muted text-[13px] mt-0.5 pointer-events-none">
        {$_("dashboard.credentials_count", { values: { count: $accounts.length } })}
      </p>
    </div>
    <button class="inline-flex items-center justify-center gap-2 h-[35px] px-4 rounded-lg text-[13px] font-medium cursor-pointer transition-all border border-transparent bg-accent text-white hover:bg-accent-hover shadow-lg shadow-accent/20" on:click={() => showAddModal.set(true)}>
      <IconPlus size={16} /> {$_("sidebar.new_credential")}
    </button>
  </div>

  <!-- Search -->
  <div class="relative mb-6">
    <div class="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-muted text-lg pointer-events-none">
      <IconSearch size={18} />
    </div>
    <input
  ...
  {#if filtered.length === 0}
    <div class="text-center py-20 px-5 flex flex-col items-center gap-3 text-text-secondary">
      <div class="text-[56px] mb-2">🔒</div>
      <h3 class="text-lg text-text-primary font-semibold">{$_("dashboard.no_credentials")}</h3>
      <p class="text-text-muted">{$_("dashboard.no_credentials_desc")}</p>
      <button class="inline-flex items-center justify-center gap-2 h-[35px] px-4 mt-2 rounded-lg text-[13px] font-medium cursor-pointer transition-all border border-transparent bg-accent text-white hover:bg-accent-hover shadow-lg shadow-accent/20" on:click={() => showAddModal.set(true)}>
        <IconPlus size={16} /> {$_("sidebar.new_credential")}
      </button>
    </div>
  {:else}
        ...
        <div class="p-5 flex flex-col gap-3.5 transition-all bg-surface/4 backdrop-blur-2xl rounded-2xl border border-surface/8 shadow-xl hover:border-accent/30 hover:translate-y-[-2px]">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              <span class="block font-semibold text-text-primary whitespace-nowrap overflow-hidden text-ellipsis">{account.site}</span>
              <span class="text-text-muted text-[12px]">@{account.username}</span>
            </div>
            <button
              ...
              <div class="flex items-center gap-0.5">
                ...
                <button
                  ...
                  data-tooltip={copyTimers[account.id] != null
                    ? $_("dashboard.clearing_in", { values: { seconds: copyTimers[account.id] } })
                    : $_("dashboard.copy_password_tooltip")}
                >
                  {#if copyTimers[account.id] != null}
                    <span class="text-[11px] font-bold min-w-[16px] text-center">{copyTimers[account.id]}</span>
                  {:else}
                    <IconCopy size={16} />
                  {/if}
                </button>

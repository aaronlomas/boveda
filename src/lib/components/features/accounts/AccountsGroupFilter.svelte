<script lang="ts">
   ...
   * @description Componente para filtrar credenciales por grupo y administrar etiquetas/grupos.
   * Totalmente desacoplado de las vistas principales mediante useGroups.
   */
  ...
  // ── Local UI State ─────────────────────────────────────────────────────────
  let editingGroup = $state<string | null>(null);   // Grupo renombrándose
  let editingValue = $state("");                     // Valor actual del rename input
  let addingGroup = $state(false);                   // Visibilidad del input nuevo grupo
  let newGroupName = $state("");                     // Valor del input nuevo grupo

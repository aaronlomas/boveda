<script lang="ts">
  ...
  async function applySolid() {
    ...
    const colorToApply =
      themeStore.bgType === "solid" ? themeStore.bgValue : "#0d0d1a";
...
<div class="grid gap-4">
  ...
  <div class="flex overflow-x-auto gap-3 pb-2">
    {#each PRESETS as preset}
      <button
        class="p-3 border transition-all flex flex-col gap-3 text-left w-30
      ...
      >
        <div class="grid grid-cols-2 rounded-xl overflow-hidden">
          <div
            class="w-full h-6 border border-surface/10"
        ...
        <span
          class="text-xs font-medium text-text-primary leading-tight text-center"
  ...
  <h1 class="text-text-primary text-xl font-bold flex items-center">
    {$_("settings.theme.text_options")}
  </h1>

  <!-- Primary Text Color -->
  <ListItem layout="double">
    <div class="flex-1">
      ...
      >
        ...
        >
          {#if themeStore.bgType === "image" && themeStore.bgValue}
            <span
              class="text-xs text-text-muted text-center px-1 leading-tight font-bold"
              >OK</span
            >
          {:else}

<script lang="ts">
  import { addAccount, generatePassword } from "$lib/utils/tauri";
  ...
  >
    ...
    <div class="flex-1 overflow-y-auto p-6 pt-4 custom-scrollbar">
      ...
      >
        ...
        <div class="flex flex-col gap-1.5">
          <label for="add-site" class="text-xs font-medium text-text-secondary"
            >{$_("add_credential.site_label")}</label
          >
          <input
            id="add-site"
        ...
        <!-- Username -->
        <div class="flex flex-col gap-1.5">
          <label for="add-user" class="text-xs font-medium text-text-secondary"
            >{$_("add_credential.user_label")}</label
          >
          <input
            id="add-user"
            class="w-full px-4 py-2.5 bg-surface/5 border border-surface/10 rounded-lg text-text-primary text-sm placeholder:text-text-muted focus:outline-none focus:border-accent/50 focus:bg-surface/8 transition-all"
        ...
        <div class="flex flex-col gap-1.5">
          <div class="flex items-center justify-between">
            <label for="add-pw" class="text-xs font-medium text-text-secondary"
              >{$_("add_credential.password_label")}</label
            >
            <button
              type="button"
            ...
            >
              {showGenerator
                ? $_("add_credential.hide")
                : $_("add_credential.generate")}
            </button>
          </div>
          <div class="relative">
            <input
          ...
          {#if showGenerator}
            ...
            >
              <div
                class="flex items-center gap-2 bg-panel/30 rounded-sm p-2.5 px-3"
              >
                <code
                  class="flex-1 font-mono text-sm text-accent-light break-all select-all"
                  >{genLoading
                    ? $_("add_credential.generating")
                    : genPreview}</code
                >
                <button
                  type="button"
        ...
        <!-- Notes -->
        <div class="flex flex-col gap-1.5">
          <label for="add-notes" class="text-xs font-medium text-text-secondary"
            >{$_("add_credential.notes_label")}</label
          >
          <textarea
            id="add-notes"
    ...
    >
      <button
        ...
        onclick={close}>{$_("add_credential.cancel_button")}</button
      >
      <button

...
>
  <div class="mb-8 flex items-center gap-4">
    <button
      class="p-2 bg-surface/5 hover:bg-surface/10 rounded-lg text-text-muted hover:text-text-primary transition-colors cursor-pointer"
  ...
  >
    <div
      class="w-16 h-16 rounded-2xl bg-accent/10 text-accent-light border border-accent/20 flex items-center justify-center mb-2"
    >
      <IconShieldLock size={32} />
    </div>
    <h3 class="text-lg text-text-primary font-semibold">En construcción</h3>
    <p class="text-text-muted max-w-sm">
      Próximamente podrás guardar o generar tokens de verificación en dos pasos
      (2FA) para tus cuentas.
    </p>
  </div>
</div>
